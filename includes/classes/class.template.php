	...
	*/
	...
	public function ParseTemplate($name = null, $return = false, $input = null){
		...
		if(empty($this->CachePath) || is_null($this->CachePath)) {
			throw new ManiTemplateException('Invalid cache path. A cache path must be specified before a template can be parsed.');
		}

		if(empty($this->TemplatePath) || is_null($this->TemplatePath)) {
			throw new ManiTemplateException('Invalid template path. A valid template path must be specified pointing to the directory of the template files.');
		}

		// check the there is something to parse, either direct input or a template file.
		if (is_null($input) && !$templateFile) {
			throw new ManiTemplateException('Nothing to parse. Either direct input is required or a valid template file must be specified.');
			return false;
		}
	 ...
	 * @param Boolean $htmlescape Specify whether or not the templating system needs to escape the variable (Default to ManiTemplate::$DefaultHtmlEscape)
	 ...
	 *
	 * @see Variables
	 * @uses ManiTemplate::$DefaultHtmlEscape
	 */
...
$TplData=str_ireplace('href="css/','href="templates/css/',$TplData);
$TplData=str_ireplace('SRC="js/','src="templates/js/',$TplData);
$TplData=str_ireplace('images/','templates/images/',$TplData);
$TplData = preg_replace('/virtual="(.*?)\.html"/simx', 'virtual="templates/$1.html"', $TplData);

preg_match_all('/<!--\s*#include\s+virtual="(.*?)\.html"\s*-->/si', $TplData, $result, PREG_PATTERN_ORDER);
for ($i = 0; $i < count($result[0]); $i++) {
	$sub_content = file_get_contents($result[1][$i].".html");
	$sub_content = str_ireplace('images/','templates/images/',$sub_content);
	$TplData = str_replace($result[0][$i],$sub_content,$TplData);
}

$TplData = preg_replace('/\{%%(.*?)\}/simx', '', $TplData);
//MANI		
		$exit = false;
		$inString = false;
		$stringChar = '';
	...
	public function ParseIncludes(){
		...
		foreach($matches[1] as $k=>$tplfile){
			if(empty($this->IncludePath) || is_null($this->IncludePath)) {
				throw new ManiTemplateException('Invalid include path. An include path must be set before {include} tokens can be used.');
			}
			if(firstchar($tplfile) === "/"){
	...
	protected function EventBeforeUncachedTemplateParsed ()
	{
		...
		return InterspireEvent::trigger('ManiTemplate_BeforeUncachedTemplateParsed', new ManiTemplateEventBeforeUncachedTemplateParsed($this));
	}
	...
	protected function EventAfterUncachedTemplateParsed ()
	{
		...
		return InterspireEvent::trigger('ManiTemplate_AfterUncachedTemplateParsed', new ManiTemplateEventAfterUncachedTemplateParsed($this));
	}
	...
	protected function EventBeforeTemplateCached (&$cacheFile)
	{
		...
		return InterspireEvent::trigger('ManiTemplate_BeforeTemplateCached', new ManiTemplateEventBeforeTemplateCached($this, $cacheFile));
	}
	...
	protected function EventAfterTemplateCached ($cacheFile)
	{
		...
		return InterspireEvent::trigger('ManiTemplate_AfterTemplateCached', new ManiTemplateEventAfterTemplateCached($this, $cacheFile));
	}
	...
	protected function EventBeforeTemplateIncluded (&$cacheFile)
	{
		...
		return InterspireEvent::trigger('ManiTemplate_BeforeTemplateIncluded', new ManiTemplateEventBeforeTemplateIncluded($this, $cacheFile));
	}
	...
	protected function EventAfterTemplateIncluded ()
	{
		...
		return InterspireEvent::trigger('ManiTemplate_AfterTemplateIncluded', new ManiTemplateEventAfterTemplateIncluded($this));
	}
	...
	protected function EventTemplateCaptured ()
	{
		...
		return InterspireEvent::trigger('ManiTemplate_TemplateCaptured', new ManiTemplateEventTemplateCaptured($this));
	}
}
...
if (class_exists('InterspireEvent') && class_exists('InterspireEventData')) {
	...
	class ManiTemplateEventBeforeUncachedTemplateParsed extends InterspireEventData
	{
		public $template;
	...
	class ManiTemplateEventAfterUncachedTemplateParsed extends InterspireEventData
	{
		public $template;
	...
	class ManiTemplateEventBeforeTemplateCached extends InterspireEventData
	{
		public $template;
	...
	class ManiTemplateEventAfterTemplateCached extends InterspireEventData
	{
		public $template;
		public $cacheFile;
	...
	class ManiTemplateEventBeforeTemplateIncluded extends InterspireEventData
	{
		public $template;
		public $cacheFile;
	...
	class ManiTemplateEventAfterTemplateIncluded extends InterspireEventData
	{
		public $template;
	...
	class ManiTemplateEventTemplateCaptured extends InterspireEventData
	{
		public $template;
...
class ManiTemplateException extends Exception { }
