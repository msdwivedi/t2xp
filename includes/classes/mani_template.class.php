<?php
/**
 * IEM implementation of Mani Template
 *
 * This adds IEM-specific functionality to the template class.
 *
 * @package @package Mani.iem.lib.iem
 */

/**
 * Include base directory
 */
#require_once(EXT_PATH . '/ext/Mani_template/class.template.php');

/**
 * EXT_ManiStash class
 *
 * The IEM Mani Stash class is designed to handle caching of commonly used variables
 ...
 * @package @package Mani.iem.lib.iem
 */
class EXT_ManiTemplate extends ManiTemplate
{
	const CACHE_LIFETIME = 432000; // Number of seconds that the cache file is valid

	/**
	 * CONSTRUCTOR
	 * @return EXT_ManiTemplate Returns an instance of this object
	 */
	...
	public function ParseTemplate($name = null, $return = false, $input = null)
	{
		...
		// ----- Initialize any variables that should be available to the running template
			$tpl = $this;
			$this->Assign('IEM', self::EXT_DefaultVariables(), false);
...
/*			if (!IEM::sessionGet('LicenseError')) {
	 ...
	 * @todo phpdoc
	 */
	private function EXT_DefaultVariables()
	{
...
/*			list($IEM['LicenseError'], $IEM['LicenseMessage']) = sesion_start();
			...
			list($IEM['MenuLinks'], $IEM['TextLinks']) = $this->EXT_Menu();*/
			$variables = $IEM;
		}
	 ...
	 *
	 * <b>Do *not* put any "ParseTemplate" calls inside EXT_Menu as you will cause an infinite loop.</b>
	 * "ParseTemplate" calls "EXT_Menu" via EXT_DefaultVariables
	 * Since the header menu has not yet finished building (ie the $menu variable is still null),
	 * calling EXT_Menu at this stage will then call ParseTemplate (which then calls EXT_Menu).
	 *
	 * It returns an array:
