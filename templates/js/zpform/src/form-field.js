// $Id: form-field.js 7944 2007-08-22 10:59:14Z andrew $
/**
 *
...
Zapatec.Form.Field.prototype.init = function(objArgs){
	...
	this.autoCompleteOptions = []; // stores values for autocompletion
	...
	this.keyPressCounter = 0; // this variable indicates how many key was pressed during last 
							  // Zapatec.Form.Field.DELAYED_INTERVAL period.
	...
	this.features = Zapatec.Form.Utils.getTokens(this.field.className, " ");
	...
	if(Zapatec.is_ie){
		Zapatec.Utils.addEvent(this.field, 'paste', onChangeFunc);
	} else if(Zapatec.is_gecko) {
		Zapatec.Utils.addEvent(this.field, 'input', onChangeFunc);
	} else {
		Zapatec.Utils.addEvent(this.field, 'change', onChangeFunc);
	}
	...
	if(this.isBooleanField){
		...
		Zapatec.Utils.addEvent(this.field, 'click', onChangeFunc);
	}
	...
	if(!this.hasFeature("zpFormMask")){
		if(this.isBooleanField){
			...
			for(var ii = elements.length - 1; ii >= 0; ii--){
				...
				if(element.name == this.field.name && element.checked){
					return false;
				}
			}
